...
$prc = (int)$book;		
$sql =" SELECT * FROM book_stack where Title LIKE \"%$book%\" OR Publisher LIKE \"$book%\"  OR  Author LIKE \"$book%\" OR Price LIKE $prc OR  Department = (SELECT Branch_Code FROM Branch WHERE Branch_Name LIKE '$book%') OR Edition = (SELECT NUM FROM Edition WHERE ALPHA LIKE '$book%' ORDER BY Title)";			
...
if( $n == 0 || $book =="") 
	{ 
		...
		echo "<tr><td colspan='13'><a href=\"searchEngin.php\"><img src='images/back.jpg' border='0'></a></td></tr></table>";
		exit(0);
	}
...
if( $i == 1 ){  $i = 2 ;  $clr = "FAEBD7";
 ...
 echo "<tr bgcolor=\"$clr\"><td  width=\"1%\" align=\"right\"><img src=\"images/serial.jpg\" width=\"20\" height=\"20\"></td><td align=\"right\">". $k++ ." </td><td align=\"center\"><input type=\"checkbox\" name=\"books\" value=\"$db_ID\"></td><td>$db_book</td><td>$db_author</td><td>$db_publish</td><td  align=\"left\">$branch</td><td  align=\"left\">$edt</td><td  align=\"right\">$db_price</td><td align='right'>$T_Books</td><td align='right'>$AB</td><td align='right'>$IB</td><td><a href=open.php?action=Open+Book&ID=$db_ID title=\" Open the list of $db_book books\" >open</a></tr>";
...
<tr><td colspan="2"><a href="searchEngin.php"><img src="images/back.jpg" border="0"></a></td><td colspan="10"><input type="hidden" name="hideBook" size="500"><input type="submit" name="Search" value="Open Book" <? if( $testBit1 < 2 ) echo "disabled=\"disabled\""; ?>  onClick="return validation()"></td></tr>
</table>
